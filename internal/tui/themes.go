...
// ocTheme is a built-in theme with a light and dark variant; applyTheme picks
...
// ocToPalette maps the OpenCode colors onto gzlab's palette, deriving the muted,
// border, and selection-background tones the source model doesn't carry.
func ocToPalette(p ocPalette) themePalette {
	// Skip a candidate equal to primary: on themes where accent==primary
	// (Cursor, Mercury dark), using it verbatim would collapse the
	// secondary/accent hierarchy into a single color across footer keys,
	// table heads, and the cursor.
	accent2 := firstHexExcluding(p.primary, p.accent, p.info, p.primary)
	return themePalette{
		accent:    lipgloss.Color(p.primary),
		secondary: lipgloss.Color(accent2),
		text:      lipgloss.Color(p.fg),
		muted:     lipgloss.Color(mixHex(p.fg, p.bg, 0.45)),
		subtle:    lipgloss.Color(mixHex(p.bg, p.fg, 0.08)),
		border:    lipgloss.Color(mixHex(p.fg, p.bg, 0.72)),
		good:      lipgloss.Color(p.good),
		warn:      lipgloss.Color(p.warn),
		bad:       lipgloss.Color(p.bad),
		info:      lipgloss.Color(firstHex(p.info, p.accent, p.primary)),
		onAccent:  lipgloss.Color(onColorFor(p.primary)),
	}
}
...
// firstHexExcluding is firstHex but skips any candidate equal to exclude.
func firstHexExcluding(exclude string, opts ...string) string {
	for _, s := range opts {
		if s != "" && s != exclude {
			return s
		}
...
// onColorFor picks a legible foreground (near-black or near-white) for text
// drawn on top of a background of the given hex color, based on perceived
// luminance — a fixed black foreground (the old behavior) vanishes against
// the dark accent hues some light themes use.
func onColorFor(hex string) string {
	r, g, b := parseHex(hex)
