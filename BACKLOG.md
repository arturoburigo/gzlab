...
| 12 — Detalhe de Merge Request | 🚧 | Branches, author, status, pipeline, approvals, conflicts, plus keys into the changed-files/diff (`d`, Épico 13), discussions (`c`, Épico 16) and commits (`C`, `glab api .../commits`) views. **Deferred:** the consolidated "Why blocked?" panel (the raw signals — pipeline, approvals, conflicts — are already shown individually). |
